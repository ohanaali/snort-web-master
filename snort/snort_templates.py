...
class snort_template():
    def __init__(self, protocol="tcp", src_port="", dest_port="any", msg="{}", nocase="", content="content",
                 rule_type="type", tag="tag: session, 10,packet;"):
    ...
    def set_rule(self, name, msg, content, treament, date, document, desc):
        ...
        content_comma = ";" if content else ""
        return f'alert {self.protocol} any any -> {self.dest_port} ( msg: "{self.msg.format(msg)}";' \
               f' {content} {content}{content_comma} sid:0; rev:1; gid: 1000000;' \
